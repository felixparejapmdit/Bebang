...
Full regression suite (settings-tab, reset-buttons, tour walkthrough, glue fractions, payroll column — 37/37 checks) re-run against the real file in its shipped (unconfigured/dormant) state: all passing, zero console errors, `node --check` clean — confirming this round is a safe, no-behavior-change push for every current user. Separately, real end-to-end verification of the *sync* behavior itself (sign-in, live cross-device updates, Security Rules actually rejecting unauthenticated access, photos confirmed absent from the synced document) is being done against a local Firebase emulator suite (Auth + Firestore, no real cloud project needed) rather than taken on faith — in progress as of this commit; results to follow.
